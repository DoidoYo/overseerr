...
export class AddFollowIdsToMedia1735431102803 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE "media" ADD COLUMN "nextEpisodeDate" datetime NULL`
    );
  }

  public async down(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `ALTER TABLE "media" DROP COLUMN "nextEpisodeDate"`
    );
    await queryRunner.query(
      `ALTER TABLE "media" DROP COLUMN "lastestFollowEvent"`
    );
    await queryRunner.query(`ALTER TABLE "media" DROP COLUMN "followingIds"`);
    await queryRunner.query(
